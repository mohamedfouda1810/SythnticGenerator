...
/* ───────────────────────────────────────────────────────────
   ...
   ─────────────────────────────────────────────────────────── */
export default function GeneratingAnimation({ status, mode, epochs = 150 }) {
  const phases = mode === 'ctgan' ? PHASES : MIMESIS_PHASES;
  const phaseMap = mode === 'ctgan' ? PHASE_MAP_CTGAN : PHASE_MAP_MIMESIS;
  const phaseIndex = phaseMap[status] ?? 0;
  const progress = Math.min(((phaseIndex + 1) / phases.length) * 100, 100);

  const rowsPercent = ['analyzing', 'training', 'generating', 'evaluating', 'complete'].includes(status) ? 100 : 0;
  const patterns = ['training', 'generating', 'evaluating', 'complete'].includes(status) ? 24 : 0;
  const epoch = ['training', 'generating', 'evaluating', 'complete'].includes(status) ? epochs : 0;
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 px-6 max-w-lg">
        ...
        {mode === 'ctgan' && (
          ...
          >
            {[
              { label: 'Analysis Progress', value: rowsPercent, suffix: '%' },
              { label: 'Patterns Found', value: patterns },
              { label: 'Total Epochs', value: epoch },
            ].map((stat) => (
              <div
                key={stat.label}
