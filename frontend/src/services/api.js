...
function getRefreshToken() {
  return useAuthStore.getState().refreshToken || localStorage.getItem('refresh_token');
}
...
async function request(url, options = {}, isRetry = false) {
  ...
  try {
    ...
    if (response.status === 401 && !isRetry) {
      // Skip refresh for login/register/refresh to avoid loops
      const skipRefreshPaths = ['/auth/login', '/auth/register', '/auth/refresh'];
      const isAuthPath = skipRefreshPaths.some(path => url.includes(path));

      if (isAuthPath) return { data: null, error: 'Unauthorized' };
...
export async function generateCTGAN(file, numRows, epochs, removePII = true) {
  const formData = new FormData();
  formData.append('file', file);
  formData.append('num_rows', String(numRows));
  formData.append('epochs', String(epochs));
  formData.append('remove_pii', removePII ? 'true' : 'false');

  return request('/generate/ctgan', {
    method: 'POST',
    body: formData,
  });
}
...
export async function downloadResult(token) {
  const { data: blob, error } = await request(`/generate/download/${token}`, {
    responseType: 'blob',
  });
