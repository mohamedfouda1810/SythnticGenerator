...
from backend.services.storage_service import store_result
...
SUPPORTED_FIELDS: List[Dict[str, Any]] = [
    ...
    {"type": "gender", "description": "Gender", "options": {}},
    {
        ...
        "options": {"start": "2000-01-01", "end": "2025-12-31"},
    },
    {
        "type": "datetime",
    ...
    },
    {
        "type": "float",
        "description": "Float",
        "options": {"min": 0.0, "max": 1000000.0, "decimals": 2},
    },
    {
        "type": "category",
...
def calculate_mimesis_quality(
    ...
    if missing_cols:
        penalty = float(min(len(missing_cols) * 10, 30))
        score -= penalty
    ...
    if null_rate > 0:
        penalty = float(round(null_rate * 30, 1))
        score -= penalty
        deductions.append(
            f"Null values: {null_rate:.1%} ({null_count} cells) (-{penalty})"
        )
    ...
    for col_name, spec in schema.items():
        ...
        if field_type in _NUMERIC_TYPES:
            ...
            if col_dtype is bool or set(df[col_name].dropna().unique()).issubset(
                {True, False, 0, 1}
            ):
                type_matches += 1
        else:
