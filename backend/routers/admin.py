...
async def list_users(
    ...
    return {
        "users": [
            {**u.to_dict(), "generation_count": gen_counts.get(u.id, 0)} for u in users
        ],
        "total": total,
        "page": page,
...
async def toggle_block(
    ...
    await log_activity(
        db, action, admin.id, {"target_user": user_id}, get_client_ip(request)
    )
    await db.commit()
...
async def change_role(
    ...
    await log_activity(
        ...
        "change_role",
        admin.id,
        {"target_user": user_id, "new_role": body.role},
...
async def delete_user(
    ...
    await log_activity(
        db, "delete_user", admin.id, {"target_user": user_id}, get_client_ip(request)
    )
    await db.commit()
...
async def get_stats(
    ...
    # Job stats
    total_gens = (
        await db.execute(select(func.count()).select_from(GenerationJob))
    ).scalar() or 0
    ctgan_count = (
        await db.execute(
            select(func.count())
            .select_from(GenerationJob)
            .where(GenerationJob.mode == GenerationMode.CTGAN)
        )
    ).scalar() or 0
    mimesis_count = (
        await db.execute(
            select(func.count())
            .select_from(GenerationJob)
            .where(GenerationJob.mode == GenerationMode.MIMESIS)
        )
    ).scalar() or 0
    completed_count = (
        await db.execute(
    ...
    gens_week = (
        await db.execute(
            select(func.count())
            .select_from(GenerationJob)
            .where(GenerationJob.created_at >= week_start)
        )
    ).scalar() or 0
    avg_quality = (
        await db.execute(
            select(func.avg(GenerationJob.quality_score)).where(
    ...
    top_users = [
        {"username": r.username, "generation_count": r.count} for r in top_result
    ]
...
async def run_cleanup(
    ...
    from backend.services.storage_service import cleanup_expired, get_store_size
