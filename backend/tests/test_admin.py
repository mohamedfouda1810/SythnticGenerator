...
from backend.models import GenerationJob, GenerationMode, JobStatus, User, UserRole
from backend.tests.conftest import register_and_verify
...
async def create_regular_user(
    client: AsyncClient, username="user1", email="user1@example.com"
):
...
async def test_block_unblock_user(client: AsyncClient):
    ...
    resp = await client.patch(
        f"/api/admin/users/{user_id}/block", headers=admin_headers
    )
    assert resp.status_code == 200
    ...
    async with db_mod.AsyncSessionLocal() as session:
        session.add(
            GenerationJob(
                ...
                num_rows_requested=10,
                error_message="Synthetic failure for test",
            )
        )
        await session.commit()

    resp = await client.get("/api/admin/errors", headers=admin_headers)
    assert resp.status_code == 200
    data = resp.json()
    assert data["total"] == 1
    assert data["errors"][0]["status"] == "failed"
    assert data["errors"][0]["error_message"] == "Synthetic failure for test"
