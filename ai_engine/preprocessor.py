...
CATEGORICAL_UNIQUE_RATIO: float = 0.10  # Increased from 5% to 10%
CATEGORICAL_MAX_UNIQUE: int = 100  # Increased from 50 to 100
...
def _is_pii_column(column_name: str, series: Optional["pd.Series"] = None) -> bool:
    ...
    # Tier 1: ID columns (always remove if they match exactly)
    if normalized in ["id", "patient_id"]:
        return True

    # Tier 2: Strict PII (substring match)
    for pattern in STRICT_PII_PATTERNS:
        if pattern in normalized:
            return True

    # Tier 3: Soft PII (exact match + type check)
    if normalized in SOFT_PII_EXACT_NAMES:
        # If it's a name, email, etc., only remove if it's string-like
...
def _detect_column_type(series: pd.Series, column_name: str = "") -> ColumnType:
    """Heuristically detect the semantic type of a pandas Series."""
    nunique = series.nunique()
    n = len(series)
    normalized = column_name.strip().lower().replace(" ", "_").replace("-", "_")

    # 0. ID check (High cardinality / Unique identifiers)
    if n > 0:
        is_id_name = (
            normalized == "id"
            or normalized.endswith("_id")
            or normalized in {"uuid", "guid", "pk"}
            or normalized.endswith(("_uuid", "_guid", "_key", "_number"))
        )

        if (nunique / n >= 0.9) and not pd.api.types.is_float_dtype(series):
            if is_id_name:
    ...
    if pd.api.types.is_numeric_dtype(series):
        if n > 0 and (
            nunique / n <= CATEGORICAL_UNIQUE_RATIO
            and nunique <= CATEGORICAL_MAX_UNIQUE
        ):
...
class DataPreprocessor:
    ...
    def fit_transform(
        ...
        for col in clean_df.columns:
            column_types[col] = _detect_column_type(clean_df[col], col)
        ...
        for col in clean_df.columns:
            ...
            sample_vals = series.dropna().head(SAMPLE_VALUES_COUNT).tolist()
    ...
    def get_sdv_metadata_dict(
        ...
        Returns:
            Dict suitable for ``sdv.metadata.Metadata.load_from_dict``.
        """
        ...
        for col_meta in preprocessor_result.columns:
            ...
            elif col_meta.dtype == ColumnType.ID:
                columns[col_meta.name] = {"sdtype": "id"}
    ...
    def _remove_pii_columns(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    def _convert_boolean_columns(
        ...
        for col, col_type in column_types.items():
            ...
            df[col] = df[col].apply(
                lambda v: (
                    (v.strip().lower() if isinstance(v, str) else v) in truthy
                    if pd.notna(v)
                    else False
                )
            )
