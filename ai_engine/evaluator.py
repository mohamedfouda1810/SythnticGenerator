...
class DataEvaluator:
    ...
    def _compute_overall_score(
        ...
        if column_metrics:
            shape_score = np.mean([m.similarity for m in column_metrics])
        else:
            shape_score = 0.0

        # Correlation score (invert diff → similarity)
        norm_diff = min(corr_diff / MAX_CORR_DIFF_NORM, 1.0)
        ...
        if missing_rates:
            ...
            avg_missing = 0.0
        missing_score = (1.0 - avg_missing) * 100.0
    ...
    def _generate_summary(
        ...
        for m in column_metrics:
            ...
            lines.append(f"  {m.column_name:<20s} {bar} {m.similarity:5.1f}%  ({m.metric_name})")
        lines.append("")
