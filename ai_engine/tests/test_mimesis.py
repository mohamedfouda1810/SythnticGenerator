...
class TestPersonalFields:
    ...
    def test_age(self, generator):
        df = generator.generate({"age": {"type": "age", "options": {"min": 20, "max": 30}}}, num_rows=100)
...
class TestNumericFields:
    def test_integer(self, generator):
        df = generator.generate({"val": {"type": "integer", "options": {"min": 0, "max": 100}}}, num_rows=50)
        assert df["val"].min() >= 0
        assert df["val"].max() <= 100

    def test_float(self, generator):
        df = generator.generate({"val": {"type": "float", "options": {"min": 0.0, "max": 1.0, "decimals": 3}}}, num_rows=50)
...
class TestCategoricalFields:
    def test_category(self, generator):
        choices = ["A", "B", "C"]
        df = generator.generate({"cat": {"type": "category", "options": {"choices": choices}}}, num_rows=100)
...
class TestFinancialFields:
    def test_salary(self, generator):
        df = generator.generate({"sal": {"type": "salary", "options": {"min": 30000, "max": 50000}}}, num_rows=50)
...
class TestEdgeCases:
    ...
    def test_large_dataset(self, generator):
        df = generator.generate({"age": {"type": "age"}, "city": {"type": "city"}}, num_rows=10_000)
        assert len(df) == 10_000

    def test_single_column(self, generator):
        df = generator.generate({"val": {"type": "integer", "options": {"min": 0, "max": 10}}}, num_rows=5)
...
class TestMimesisRequest:
    def test_generate_from_request(self, generator):
        req = MimesisRequest(
            schema={"patient": MimesisFieldSpec(type=MimesisFieldType.NAME), "age": MimesisFieldSpec(type=MimesisFieldType.AGE)},
            num_rows=25,
        )
